...
Stat.prototype = {startDate:null, endDate:null, maxScore:0, considerationWeights:[], lastActionOtherOrganismSize:0, lastActionOtherOrganismDist:0, lastActionOtherOrganismPv:0, lastActionMyOrganismPv:0, lastActionMyOrganismSize:0, cushion:0, ping:0, avgPing:0};
function Organism() {
}
Organism.prototype = {nx:0, ny:0, dx:0, dy:0, v:0, cushion:0, size:0, isVirus:false};
...
Action.prototype = {type:"", x:0, y:0, myOrganism:null, otherOrganism:null, weightedValues:[], calcImportance:function(considerations) {
    var filteredConsiderations = considerations.filter(function(consideration) {
        return consideration.weight && consideration.filter(this.myOrganism, this.otherOrganism, this);
    }, this);
    if (filteredConsiderations.length) {
        ...
        return this.weightedValues[0][0] / filteredConsiderations.map(function(consideration) {
...
Consideration.prototype = {weight:1, label:"", get value() {
    return ~~this.weight;
}, calc:function(myOrganism, otherOrganism, action) {
}, min:0, max:0, weightedCalc:function(myOrganism, otherOrganism) {
    ...
    ctx.beginPath();
    ctx.arc(otherOrganism.nx, otherOrganism.ny, ~~Math.pow(Math.pow(otherOrganism.nx - myOrganism.nx, 2) + Math.pow(otherOrganism.ny - myOrganism.ny, 2), .5), 0, 2 * Math.PI);
    ctx.stroke();
}};
...
var actionGeneratorPrototype = {genAction:function(myOrganism, otherOrganism) {
}};
    ...
    chrome.storage.local.get(["gameHistory5", "exp"], function(items) {
        if (items.gameHistory5) {
            ...
            for (var i = 0;i < items.gameHistory5.length;i++) {
                var stat = items.gameHistory5[i], totalWeight = stat.considerationWeights.reduce(function(a, b) {
                    return a + b;
...
Ai.prototype = {score:0, exp:0, get lvl() {
    return ~~Math.pow(this.exp, .25);
}, get lvlPercent() {
    return ~~((this.exp - Math.pow(this.lvl, 4)) / (Math.pow(this.lvl + 1, 4) - Math.pow(this.lvl, 4)) * 100);
}, name:"ZYMO", totalWeightedScore:0, cushions:[], pings:[], splitCooldown:1E4, depth:3, onDraw:function() {
}, totalWeights:[], allowIntuition:false, lastActionBest30:[], lastActionBest5:[], cushion:0, heatmapEnabled:false, actionGenerators:[new ActionGenerator("Intercept small blob", function(myOrganism, otherOrganism, specialNames) {
    ...
    return new Action("move", otherOrganism.nx + otherOrganism.dx * tickCount, otherOrganism.ny + otherOrganism.dy * tickCount, myOrganism, otherOrganism);
}, [new Consideration("Chase Blob w/ Slightly Smaller Mass", function() {
    return true;
}, function(myOrganism, otherOrganism) {
    return otherOrganism.size - myOrganism.size;
}), new Consideration("Chase Nearest Smaller Blob", function() {
    return true;
}, function(myOrganism, otherOrganism) {
    return -Math.pow(Math.pow(myOrganism.nx - otherOrganism.nx, 2) + Math.pow(myOrganism.ny - otherOrganism.ny, 2), .2);
}), new Consideration("Chase Smaller Blobs near Edge", function(myOrganism, otherOrganism) {
    return otherOrganism.v;
}, function(myOrganism, otherOrganism) {
    return Math.pow(5600 - otherOrganism.nx, 2) + Math.pow(5600 - otherOrganism.ny, 2);
})]), new ActionGenerator("Juke big blob", function(myOrganism, otherOrganism) {
    return !otherOrganism.isVirus && otherOrganism.size * .85 > myOrganism.size;
}, function(myOrganism, otherOrganism) {
    return true;
}, function(myOrganism, otherOrganism) {
    var tickCount = otherOrganism.v ? Math.pow(Math.pow(myOrganism.nx - otherOrganism.nx, 2) + Math.pow(myOrganism.ny - otherOrganism.ny, 2), .5) / 2 / otherOrganism.v : 0;
    return new Action("move", myOrganism.nx * 2 - otherOrganism.nx - otherOrganism.dx * tickCount, myOrganism.ny * 2 - otherOrganism.ny - otherOrganism.dy * tickCount, myOrganism, otherOrganism);
}, [new Consideration("Avoid Blob w/ Slightly Larger Mass", function() {
    return true;
}, function(myOrganism, otherOrganism) {
    return -Math.abs(myOrganism.size - otherOrganism.size);
}), new Consideration("Avoid Nearest Larger Blob", function() {
    return true;
}, function(myOrganism, otherOrganism) {
    return -Math.pow(Math.pow(otherOrganism.nx - myOrganism.nx, 2) + Math.pow(otherOrganism.ny - myOrganism.ny, 2), .1);
}), new Consideration("Avoid Bigger Blobs near Edge", function() {
    return true;
}, function(myOrganism, otherOrganism) {
    return Math.pow(5600 - otherOrganism.nx, 2) + Math.pow(5600 - otherOrganism.ny, 2);
}), new Consideration("Avoid Splitters", function(myOrganism, otherOrganism) {
    return otherOrganism.size > 63 && myOrganism.size < otherOrganism.size * .425;
}, function(myOrganism, otherOrganism) {
    return myOrganism.size - otherOrganism.size;
}), new Consideration("Avoid when split", function() {
    return true;
}, function() {
    return Ai.prototype.myOrganisms.length;
})]), new ActionGenerator("B line away from virus", function(myOrganism, otherOrganism) {
    return otherOrganism.isVirus && otherOrganism.size < myOrganism.size * .85;
}, function(myOrganism, otherOrganism) {
    return true;
}, function(myOrganism, otherOrganism) {
    return new Action("move", myOrganism.nx * 2 - otherOrganism.nx, myOrganism.ny * 2 - otherOrganism.ny, myOrganism, otherOrganism);
}, [new Consideration("Avoid Virus Attackers", function(myOrganism, otherOrganism) {
    return Math.pow(Math.pow(otherOrganism.nx - myOrganism.nx, 2) + Math.pow(otherOrganism.ny - myOrganism.ny, 2), .5) < 660;
}, function(myOrganism, otherOrganism) {
    return myOrganism.size - otherOrganism.size;
}), new Consideration("Avoid Colliding into Virus", function(myOrganism, otherOrganism) {
    ...
    return true;
}), new Consideration("Avoid Nearest Virus", function() {
    return true;
}, function(myOrganism, otherOrganism) {
    return -Math.pow(Math.pow(otherOrganism.nx - myOrganism.nx, 2) + Math.pow(otherOrganism.ny - myOrganism.ny, 2), .1);
})]), new ActionGenerator("Split small blob", function(myOrganism, otherOrganism) {
    var dist = Math.pow(Math.pow(myOrganism.nx - otherOrganism.nx, 2) + Math.pow(myOrganism.ny - otherOrganism.ny, 2), .5), ftrDist = Math.pow(Math.pow(myOrganism.nx - otherOrganism.nx - otherOrganism.dx, 2) + Math.pow(myOrganism.ny - otherOrganism.ny - otherOrganism.dy, 2), .5);
    return !otherOrganism.isVirus && otherOrganism.v && otherOrganism.size > 48 && otherOrganism.size < myOrganism.size * .425 && dist < ftrDist && dist < 500 - myOrganism.size && Ai.prototype.allowSplit;
}, function(myOrganism, otherOrganism) {
    return true;
}, function(myOrganism, otherOrganism) {
    ...
    return new Action("split", otherOrganism.nx + otherOrganism.dx * tickCount, otherOrganism.ny + otherOrganism.dy * tickCount, myOrganism, otherOrganism);
}, [new Consideration("Split on smaller blob", function() {
    return true;
}, function(myOrganism, otherOrganism) {
    return otherOrganism.size - myOrganism.size;
}), new Consideration("Split on farther blob", function() {
    return true;
}, function(myOrganism, otherOrganism) {
    return Math.pow(otherOrganism.nx - myOrganism.nx, 2) + Math.pow(otherOrganism.ny - myOrganism.ny, 2);
})]), new ActionGenerator("Shoot to bait", function(myOrganism, otherOrganism) {
    var dist = Math.pow(Math.pow(myOrganism.nx - otherOrganism.nx, 2) + Math.pow(myOrganism.ny - otherOrganism.ny, 2), .5), ftrDist = Math.pow(Math.pow(myOrganism.nx - otherOrganism.nx - otherOrganism.dx, 2) + Math.pow(myOrganism.ny - otherOrganism.ny - otherOrganism.dy, 2), .5);
    return !otherOrganism.isVirus && otherOrganism.v && otherOrganism.size > 48 && otherOrganism.size < myOrganism.size * .425 && dist < ftrDist && dist < 750 - myOrganism.size && dist > 650 - myOrganism.size && Ai.prototype.allowShoot;
}, function(myOrganism, otherOrganism) {
    return true;
}, function(myOrganism, otherOrganism) {
    var tickCount = Math.pow(Math.pow(myOrganism.nx - otherOrganism.nx, 2) + Math.pow(myOrganism.ny - otherOrganism.ny, 2), .5) / 2 / otherOrganism.v;
    return new Action("shoot", otherOrganism.nx - otherOrganism.dx * tickCount, otherOrganism.ny - otherOrganism.dy * tickCount, myOrganism, otherOrganism);
}, [new Consideration("Shoot to bait", function() {
    return true;
}, function() {
    return true;
})])], linesEnabled:true, lastAction:null, currentState:"", lastStateChangeDate:null, gameHistory:[], scoreHistory:[], myOrganisms:[], otherOrganisms:[], allowSplit:true, allowShoot:true, shootCooldown:5E3, onDeath:function() {
}, myRing:["rgba(255,255,255,.95)", 10, "rgba(53,255,255,.3)", 20], badRing:["rgba(255,00,00,1)", 4, "rgba(231,00,00,.3)", 20], goodRing:["rgba(255,255,255,.95)", 2, "rgba(12,227,172,.3)", 10], otherRing:["rgba(255,255,255,.9)", 1, "rgba(12,227,172,.1)", 2], simulateAction:function(myOrganism, otherOrganisms, action) {
    var tickCount = myOrganism.v ? Math.pow(Math.pow(action.x - myOrganism.nx, 2) + Math.pow(action.y - myOrganism.ny, 2), .5) / myOrganism.v : 0, clonedMyOrganism = new Organism;
    Object.keys(myOrganism).forEach(function(key) {
        clonedMyOrganism[key] = myOrganism[key];
    });
    clonedMyOrganism.nx = action.x;
    clonedMyOrganism.ny = action.y;
    return {myOrganism:clonedMyOrganism, otherOrganisms:otherOrganisms.filter(function(a) {
        return a.id != action.otherOrganism.id;
    }).map(function(organism) {
        var clone = new Organism;
        Object.keys(organism).forEach(function(key) {
            clone[key] = organism[key];
        });
...
}, actionCooldown:0, tick:function(organisms, myOrganisms, score) {
    if (score > this.score) {
    ...
    if (myOrganisms.length) {
        var mergedOrganism = new Organism, totalSize = myOrganisms.map(function(myOrganism) {
            return myOrganism.size;
        }).reduce(function(a, b) {
            return a + b;
        });
        mergedOrganism.src = myOrganisms;
        Object.keys(Organism.prototype).forEach(function(key) {
            var totalValue = myOrganisms.map(function(myOrganism) {
                return myOrganism[key] * myOrganism.size;
            }).reduce(function(a, b) {
                return a + b;
            });
        ...
        if (action && (this.actionCooldown < 1 || action.myOrganism.size < action.otherOrganism.size * .85 || action.myOrganism.src.length > 1 || action.otherOrganism.v)) {
            if (action.myOrganism.size < action.otherOrganism.size * .85 && Math.pow(Math.pow(action.x - 5600, 2) + Math.pow(action.y - 5600, 2), .5) > 5600) {
            ...
            switch(action.type) {
                ...
                case "split":
                    ...
                    setTimeout(function() {
                        Ai.prototype.allowSplit = true;
                    }.bind(this), this.splitCooldown);
                    this.split();
                    break;
                case "shoot":
                    this.move(~~action.x, ~~action.y);
                    ...
                    setTimeout(function() {
                        Ai.prototype.allowShoot = true;
                    }.bind(this), this.shootCooldown);
                    this.shoot();
                    break;
            }
            if (!action.otherOrganism.v && action.myOrganism.size * .85 > action.otherOrganism.size) {
                this.actionCooldown = action.myOrganism.v ? ~~((Math.pow(Math.pow(action.myOrganism.nx - action.otherOrganism.nx, 2) + Math.pow(action.myOrganism.ny - action.otherOrganism.ny, 2), .5) - action.myOrganism.size) / action.myOrganism.v / 2) : 0;
            } else {
        ...
        if (this.currentState != "alive") {
            ...
            this.avgPing = this.pings.reduce(function(a, b, i) {
                    return a + b * Math.pow(2, i);
                }) / (this.pings.map(function(a, i) {
                    return Math.pow(2, i);
                }).reduce(function(a, b) {
                    return a + b;
                }) + 1);
        ...
        if (this.currentState == "alive") {
            var considerationWeights = this.actionGenerators.map(function(actionGenerator) {
                return actionGenerator.considerations;
            }).reduce(function(a, b) {
                return a.concat(b);
            }).map(function(consideration) {
                return consideration.weight;
            }), stat = new Stat(this.lastStateChangeDate, new Date, Math.max.apply(null, this.scoreHistory), considerationWeights);
            if (this.lastAction) {
                ...
                if (mOrganism.size < oOrganism.size * .85 && oOrganism.size - oOrganism.cushion / 2 > stat.lastActionOtherOrganismDist) {
                    ...
                    Ai.prototype.cushion = this.cushions.reduce(function(a, b, i) {
                            return a + b * i;
                        }) / (this.cushions.map(function(a, i) {
                            return i;
                        }).reduce(function(a, b) {
                            return a + b;
                        }) + 1);
            ...
            chrome.storage.local.set({gameHistory5:slicedGameHistory, exp:this.exp});
            var weights = this.totalWeights;
            for (var i = this.gameHistory.length - 1;i < this.gameHistory.length;i++) {
                var stat = this.gameHistory[i], totalWeight = stat.considerationWeights.reduce(function(a, b) {
                    return a + b;
                });
                for (var j = 0;j < stat.considerationWeights.length;j++) {
            ...
            if (this.allowIntuition) {
                ...
                var avgWeight = weights.reduce(function(a, b) {
                        return a + b;
                    }) / weights.length;
    ...
    if (otherOrganism.name != "Best route" && !otherOrganism.isVirus && myOrganism.size < otherOrganism.size * .85 && dist < (otherOrganism.cushion + myOrganism.cushion + Ai.prototype.cushion) * 3 + otherOrganism.size) {
        var action = new Action("", myOrganism.nx * 2 - otherOrganism.nx, myOrganism.ny * 2 - otherOrganism.ny, myOrganism, otherOrganism);
        action.importance = 5;
        if (Ai.prototype.allowSplit && dist < otherOrganism.cushion + myOrganism.cushion + Ai.prototype.cushion + otherOrganism.size) {
            action.type = "split";
            return [action];
        } else {
            if (Ai.prototype.allowShoot && dist < (otherOrganism.cushion + myOrganism.cushion + Ai.prototype.cushion) * 2 + otherOrganism.size) {
                action.type = "shoot";
                return [action];
            } else {
                if (dist < (otherOrganism.cushion + myOrganism.cushion + Ai.prototype.cushion) * 3 + otherOrganism.size) {
                    action.type = "move";
                    return [action];
                }
            }
    ...
    return this.actionGenerators.filter(function(actionGenerator) {
        return actionGenerator.filter(myOrganism, otherOrganism);
    ...
    if (actions.length > 1) {
        var virusTotalImportance = 0, organismTotalImportance = 0, organism = new Organism, virus = new Organism, virusSrcActions = [], organismSrcActions = [];
        actions.filter(function(action) {
            return action.type == "move" && action.otherOrganism.isVirus && myOrganism.size * .85 > action.otherOrganism.size;
        ...
        if (virusTotalImportance) {
            ...
            virus.isVirus = true;
            actions = actions.concat(this.genActions(myOrganism, virus).map(function(a) {
                a.srcActions = virusSrcActions;
                return a;
            }));
        }
        actions.filter(function(action) {
            return action.type == "move" && !action.otherOrganism.isVirus && myOrganism.size < action.otherOrganism.size * .85;
        }).forEach(function(action) {
        ...
        if (organismTotalImportance) {
            ...
            actions = actions.concat(this.genActions(myOrganism, organism).map(function(a) {
                a.srcActions = organismSrcActions;
                return a;
            }));
        }
    }
    ...
    return actions[0];
}, heatMapType:6, draw:function(ctx) {
    ...
    if (this.heatmapEnabled && this.lastAction && this.lastAction.myOrganism && this.lastAction.myOrganism.src && this.lastAction.myOrganism.src.length) {
        var mOrganism = this.lastAction.myOrganism.src[0], size = ~~mOrganism.size * 2;
        if ([3, 4, 9, 11].indexOf(this.heatMapType) != -1) {
            for (var x = mOrganism.x - 12 * size;x < mOrganism.x + 12 * size;x += size) {
                for (var y = mOrganism.y - 10 * size;y < mOrganism.y + 10 * size;y += size) {
                    ...
                    cOrganism.nx = x;
                    cOrganism.ny = y;
                    var value = Math.max.apply(null, this.otherOrganisms.filter(function(oOrganism) {
                            return this.considerations[this.heatMapType].filter(cOrganism, oOrganism, {type:"move"});
                        }, this).map(function(oOrganism) {
            ...
            if ([0, 1, 2, 8, 10].indexOf(this.heatMapType) != -1) {
                this.otherOrganisms.filter(function(oOrganism) {
                    return this.considerations[this.heatMapType].filter(mOrganism, oOrganism, {type:"move"});
                }, this).forEach(function(oOrganism) {
                ...
                if ([6, 7].indexOf(this.heatMapType) != -1) {
                    for (var x = mOrganism.x - 12 * size;x < mOrganism.x + 12 * size;x += size) {
                        for (var y = mOrganism.y - 10 * size;y < mOrganism.y + 10 * size;y += size) {
                            var cOrganism = new Organism;
                            Object.keys(mOrganism).forEach(function(key) {
            ...
            if (lastAction) {
                ...
                if (lastAction.srcActions) {
                    for (var i = 0;i < lastAction.srcActions.length;i++) {
                        ctx.beginPath();
                        ctx.arc(lastAction.srcActions[i].otherOrganism.nx, lastAction.srcActions[i].otherOrganism.ny, lastAction.srcActions[i].otherOrganism.size + ~~lastAction.srcActions[i].otherOrganism.cushion, 0, 2 * Math.PI);
                        ctx.stroke();
                    }
                    ctx.lineWidth /= 2;
                    for (var i = 0;i < lastAction.srcActions.length;i++) {
                        ctx.beginPath();
                        ctx.arc(lastAction.srcActions[i].otherOrganism.nx, lastAction.srcActions[i].otherOrganism.ny, lastAction.srcActions[i].otherOrganism.size, 0, 2 * Math.PI);
                        ctx.stroke();
                        ctx.beginPath();
                ...
                if (lastAction.myOrganism.src.length == 1) {
                    ...
                    ctx.arc(organism.x, organism.y, organism.size + ~~lastAction.myOrganism.cushion, 0, 2 * Math.PI);
                } else {
                    ctx.arc(lastAction.myOrganism.nx, lastAction.myOrganism.ny, lastAction.myOrganism.size + ~~lastAction.myOrganism.cushion, 0, 2 * Math.PI);
                }
                ctx.stroke();
                ctx.lineWidth /= 2;
                ctx.beginPath();
                if (lastAction.myOrganism.src.length == 1) {
                    ...
                    ctx.arc(lastAction.myOrganism.nx, lastAction.myOrganism.ny, lastAction.myOrganism.size, 0, 2 * Math.PI);
                }
                ctx.stroke();
            }
        }
